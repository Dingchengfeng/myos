...
void sheet_slide(struct SHEET *sht, int vx0, int vy0)
{
	...
	sht->vy0 = vy0;
	if (sht->height >= 0) { /*  如果正在显示,按新图层的信息刷新画面 */
		sheet_refreshsub(sht->ctl, old_vx0, old_vy0, old_vx0 + sht->bxsize, old_vy0 + sht->bysize);//原位置刷新一遍
		sheet_refreshsub(sht->ctl, vx0, vy0, vx0 + sht->bxsize, vy0 + sht->bysize);//新位置刷新一遍
	}
	return;
}
//释放图层
