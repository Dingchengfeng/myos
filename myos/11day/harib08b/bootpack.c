...
void HariMain(void)
{
	...
	sheet_slide(shtctl, sht_back, 0, 0); //设置鼠标初始位置
	mx = (binfo->scrnx - 16) / 2; //16是鼠标本身的长
	my = (binfo->scrny - 28 - 16) / 2;//16是鼠标本身的宽，28是底部导航栏的宽
	sheet_slide(shtctl, sht_mouse, mx, my);//设置鼠标位置
	sheet_updown(shtctl, sht_back,  0); //背景图层在0层,显示刷新
	sheet_updown(shtctl, sht_mouse, 1); //鼠标图层在1层,显示刷新
	...
	sheet_refresh(shtctl, sht_back, 0, 0, binfo->scrnx, 48); //刷新显示
	...
	for (;;) {//先处理键盘，如果键盘为空在处理鼠标
		...
		if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) == 0) {
			...
			if (fifo8_status(&keyfifo) != 0) {
				...
				sheet_refresh(shtctl, sht_back, 0, 16, 16, 32);//刷新显示
