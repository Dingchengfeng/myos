...
GDT0:							;最初的GDT 在asmhead里面，即haribote.sys的内存区域
		...
		DW		0xffff,0x0000,0x9200,0x00cf	; 可读写的段（segment）32bit 1号段 基址是0x00000000 段上限是fffff,GB标志=1，故段大小=(FFFFF+1)*4K=4G
		DW		0xffff,0x0000,0x9a28,0x0047	; 可执行的段 32bit（bootpack用） 2号段 基址是0x00280000 段上限是7ffff，故段大小是512K（7ffff+1）

		DW		0
GDTR0:
		DW		8*3-1		;16位段上限  段表容量-1（字节数），这里容纳3个段记录，每段8字节;0x0000 0000 0000 10111;高13位值为2,表示最大是二号段
